...
export function ModerationB2CPage() {
  ...
  const listRows = useMemo(() => (lists.data ?? []).filter((l) => !isOgolne(l)), [lists.data]);
  const listGroups = useMemo(() => groupByUser(listRows), [listRows]);
  const tripGroups = useMemo(() => groupByUser(trips.data ?? []), [trips.data]);
  ...
  return (
    ...
    <div className="max-w-3xl">
      ...
      {seg === "wyjazdy"
        ...
        : lists.isLoading ? <Spin /> : lists.isError ? <ErrMsg /> : listGroups.length === 0
          ? <Empty text="Brak list." />
          : listGroups.map((g) => (
              <UserSection key={g.user_id} group={g} noun="lista" nounPl="listy" nounGen="list">
...
// ── Sekcja jednego uzytkownika (awatar + @username + wszystkie jego tresci) ──
function UserSection<T>({ group, children, noun, nounPl, nounGen }: { group: UserGroup<T>; children: ReactNode; noun: string; nounPl: string; nounGen: string }) {
  ...
  return (
    <section className="mb-6">
      <div className="flex items-center gap-2.5 mb-2 px-1">
        {group.avatar
          ? <img src={avatarSrc(group.avatar)} alt="" className="h-8 w-8 rounded-full object-cover bg-slate-100" />
          : <span className="h-8 w-8 rounded-full bg-slate-200 text-slate-500 text-xs font-bold flex items-center justify-center">{uname.slice(0, 1).toUpperCase()}</span>}
        <div className="min-w-0">
          <p className="text-sm font-bold text-slate-900 truncate">@{uname}</p>
          <p className="text-[11px] text-slate-400">{n} {n === 1 ? noun : n >= 2 && n <= 4 ? nounPl : nounGen}</p>
        </div>
      </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {trip.hidden_by_admin && <span className="shrink-0 px-2 py-0.5 rounded-full text-[11px] font-bold bg-slate-800 text-white">Ukryte</span>}
      </div>
    ...
    <div className="mt-3 rounded-xl bg-slate-50 p-3 space-y-3">
      {places.map((p, i) => (
        <div key={i} className="border-b border-slate-100 last:border-0 pb-3 last:pb-0">
          <p className="text-sm font-semibold text-slate-800 truncate">{p.place_name}</p>
          {p.photos.length > 0 && (
