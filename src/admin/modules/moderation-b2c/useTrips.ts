...
export interface TripCol {
  ...
  place_count: number;
  thumbs: string[];        // miniaturki okladek miejsc (zdjecia userow)
  ...
  return useQuery<TripCol[]>({
    ...
    queryFn: async () => {
      ...
      if (uids.length) {
        ...
        (profs ?? []).forEach((p: any) => { usernames[p.id] = p.username || null; avatars[p.id] = p.avatar_url || null; });
      }
      return routes.map((r) => ({
        ...r, place_count: counts[r.id] ?? 0, thumbs: thumbs[r.id] ?? [],
        username: usernames[r.user_id] ?? null, avatar: avatars[r.user_id] ?? null,
      }));
    },
  });
}
