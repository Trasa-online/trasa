...
function DemoSwiper({ places, onComplete }: { places: DemoPlace[]; onComplete: (liked: DemoPlace[]) => void }) {
  ...
  return (
    <div className="flex-1 flex flex-col px-4 pb-6 pt-3 gap-4">
      {/* Progress */}
      <div className="flex gap-1">
        {places.map((_, i) => (
          <div key={i} className={cn("flex-1 h-1 rounded-full transition-colors", i <= index ? "bg-orange-600" : "bg-border/40")} />
        ))}
      </div>
      ...
      <div
        className={cn(
          "flex-1 rounded-3xl overflow-hidden bg-card border border-border/20 shadow-lg flex flex-col transition-all duration-200",
          animating === "right" && "translate-x-8 rotate-3 opacity-0",
          animating === "left" && "-translate-x-8 -rotate-3 opacity-0"
        )}
        ...
        onTouchEnd={e => {
          ...
          if (Math.abs(dx) > 60) advance(dx > 0);
        }}
      >
        <div className="relative h-56 shrink-0">
          ...
          <div className="absolute bottom-3 left-4 right-4">
            <p className="text-white font-black text-xl leading-tight">{current.name}</p>
            <div className="flex items-center gap-1 mt-0.5">
              <Star className="h-3 w-3 fill-yellow-400 text-yellow-400" />
              <span className="text-white/90 text-sm font-semibold">{current.rating}</span>
              <span className="text-white/60 text-xs ml-1">{current.address}</span>
            </div>
          </div>
        </div>
        <div className="flex-1 px-4 py-3 space-y-2">
          ...
          <div className="flex gap-1.5 flex-wrap">
            {current.tags.map(tag => (
              <span key={tag} className="px-2 py-0.5 rounded-full bg-muted text-xs text-muted-foreground font-medium">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div className="flex gap-4 justify-center">
        <button
          ...
          className="h-16 w-16 rounded-full border-2 border-border/60 bg-card shadow-md flex items-center justify-center active:scale-95 transition-transform"
        >
          <X className="h-7 w-7 text-muted-foreground" />
        </button>
        <button
          onClick={() => advance(true)}
          className="h-16 w-16 rounded-full bg-orange-600 shadow-md shadow-orange-600/30 flex items-center justify-center active:scale-95 transition-transform"
        >
      ...
      <p className="text-center text-xs text-muted-foreground">
        {index + 1} / {places.length} — możesz też swipe'ować kartę
      </p>
    </div>
  );
}
