-- Podziekowanie od lokalu: RPC + push. Czesc druga (enum wszedl migracja 20260915f).
--
-- Po co to jest: notatka goscia to jedyna waluta zaufania, ktora mamy - nie ma gwiazdek
-- ani ocen do kupienia. Lokal, ktory moze za nia podziekowac, zamyka petle: czlowiek widzi,
-- ze jego zdanie doszlo do adresata. To jedyny kanal lokal -> uzytkownik w calej aplikacji,
-- wiec jest waski z premedytacja: jedno podziekowanie za ta sama tresc i limit na dobe.
--
-- ⛔ Lokal NIE dostaje `user_id` autora notatki. RPC przyjmuje NAZWE uzytkownika (ta, ktora
-- lokal i tak widzi przy notatce) i sam ja rozwiazuje - panel nigdy nie trzyma identyfikatora
-- osoby, ktorej nie zna.

create or replace function public.notify_business_thanks(
  p_business_id uuid,
  p_username    text,
  p_kind        text default 'note',   -- 'note' | 'photo'
  p_ref         text default null      -- klucz konkretnej tresci (dedup)
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select id, business_name, logo_url, place_id, owner_user_id
    into v_biz
    from public.business_profiles
   where id = p_business_id;

  if v_biz.id is null then
    raise exception 'Nie ma takiej wizytowki';
  end if;
  -- Dziekowac moze WYLACZNIE wlasciciel wizytowki. Bez tego kazde zalogowane konto
  -- mogloby wysylac powiadomienia "od lokalu" dowolnej osobie.
  if v_biz.owner_user_id is distinct from v_uid then
    raise exception 'Tylko wlasciciel lokalu moze dziekowac';
  end if;
  ...
  if v_target is null then
    return jsonb_build_object('sent', false, 'reason', 'no_user');
  end if;
  if v_target = v_uid then
    return jsonb_build_object('sent', false, 'reason', 'self');
  end if;

  -- Za te sama tresc dziekujemy RAZ (okno 90 dni). Powtorka nie jest bledem - po prostu
  -- nic nie wysylamy i mowimy o tym panelowi.
  select count(*) into v_dupes
    from public.notifications n
   where n.user_id = v_target
     and n.type = 'business_thanks'
     and n.metadata->>'business_id' = p_business_id::text
     and coalesce(n.metadata->>'ref', '') = coalesce(p_ref, '')
     and n.created_at > now() - interval '90 days';
  if v_dupes > 0 then
    return jsonb_build_object('sent', false, 'reason', 'already');
  end if;

  -- Bezpiecznik: 30 podziekowan na dobe z jednego lokalu. Kanal lokal -> user musi byc
  -- odporny na pomysl "podziekujmy wszystkim naraz".
  select count(*) into v_today
    from public.notifications n
   where n.type = 'business_thanks'
     and n.metadata->>'business_id' = p_business_id::text
     and n.created_at > now() - interval '24 hours';
  if v_today >= 30 then
    return jsonb_build_object('sent', false, 'reason', 'limit');
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'place_id',      v_biz.place_id,
      'kind',          coalesce(p_kind, 'note'),
      'ref',           p_ref
    )
  );
...
-- Supabase nadaje EXECUTE roli anon z automatu; samo `revoke from public` tego nie zdejmuje.
revoke all on function public.notify_business_thanks(uuid, text, text, text) from public, anon;
grant execute on function public.notify_business_thanks(uuid, text, text, text) to authenticated;

comment on function public.notify_business_thanks(uuid, text, text, text) is
  'Podziekowanie lokalu za notatke/zdjecie goscia. Tylko wlasciciel wizytowki, raz na tresc, max 30/dobe.';

-- Push dla nowego typu: ta sama funkcja co dla reszty powiadomien, nowa galaz tresci.
...
BEGIN
  IF NEW.type NOT IN ('group_invite','friend_request','friend_accept','route_used',
                      ...
                      'route_hidden','list_invite','business_thanks') THEN
    RETURN NEW;
  END IF;
  ...
  ELSIF NEW.type = 'business_thanks' THEN
    IF v_en THEN
      v_title := 'Thank-you from a venue';
      v_body  := COALESCE(NEW.metadata->>'business_name', 'A venue')
                 || CASE WHEN COALESCE(NEW.metadata->>'kind','note') = 'photo'
                      THEN ' says thanks for your photo' ELSE ' says thanks for your note' END;
    ELSE
      v_title := 'Podziękowanie od lokalu';
      v_body  := COALESCE(NEW.metadata->>'business_name', 'Lokal')
                 || CASE WHEN COALESCE(NEW.metadata->>'kind','note') = 'photo'
                      THEN ' dziękuje za Twoje zdjęcie' ELSE ' dziękuje za Twoją notatkę' END;
    END IF;
    -- Wlasne notatki i zdjecia czlowiek ma u siebie na profilu - tam go odsylamy.
    v_url   := '/moj-profil';
...
-- Za co lokal juz podziekowal. Panel musi to wiedziec PO ODSWIEZENIU strony, inaczej guzik
-- wraca do stanu „Podziękuj" i lokal klika w prozne (RPC i tak odmowi, ale wyglada to na blad).
-- ⛔ Zwracamy WYLACZNIE klucze tresci, zadnych odbiorcow - lokal nie ma prawa wiedziec, kto
-- dokladnie dostal powiadomienie.
create or replace function public.business_thanks_refs(p_business_id uuid)
returns text[]
language plpgsql
security definer
set search_path to 'public', 'auth'
...
begin
  ...
  select owner_user_id into v_owner from public.business_profiles where id = p_business_id;
  if v_owner is distinct from v_uid then
    raise exception 'Tylko wlasciciel lokalu';
  end if;

  select coalesce(array_agg(distinct n.metadata->>'ref') filter (where n.metadata->>'ref' is not null), '{}')
    into v_refs
    from public.notifications n
   where n.type = 'business_thanks'
     and n.metadata->>'business_id' = p_business_id::text
...
revoke all on function public.business_thanks_refs(uuid) from public, anon;
grant execute on function public.business_thanks_refs(uuid) to authenticated;
