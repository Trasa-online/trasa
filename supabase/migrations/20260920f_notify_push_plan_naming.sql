-- Rename wyjazd -> plan w copy pushy (decyzja Nat 2026-09-20, pkt 4: 'naming wyjazdy > plany').
-- Sama tresc literalow; logika, adresy (/moj-profil?tab=wyjazdy) i galezie bez zmian.
...
BEGIN
  IF NEW.type NOT IN ('group_invite','friend_request','friend_accept','route_used',
                      ...
                      'route_hidden','list_invite','business_thanks','list_starred') THEN
    RETURN NEW;
  END IF;
  ...
  ELSIF NEW.type = 'route_hidden' THEN
    ...
    IF v_en THEN
      v_title := 'Your plans are not visible yet';
      v_body  := CASE WHEN v_added = 1
                   THEN 'One published plan has no photo, so nobody can find it in Explore. Add one and it shows up.'
                   ELSE v_added || ' published plans have no photo, so nobody can find them in Explore. Add one and they show up.' END;
    ELSE
      v_title := 'Twoich planów jeszcze nie widać';
      v_body  := CASE WHEN v_added = 1
                   THEN 'Jeden opublikowany plan nie ma zdjęcia, więc nikt go nie znajdzie w eksploracji. Dodaj jedno i się pojawi.'
                   ELSE v_added || ' opublikowane plany nie mają zdjęcia, więc nikt ich nie znajdzie w eksploracji. Dodaj po jednym i się pojawią.' END;
    END IF;
    v_url   := '/moj-profil?tab=wyjazdy';

  ELSIF NEW.type = 'route_invite' THEN
    IF v_en THEN
      v_title := 'New shared plan 🗺️';
      v_body  := v_actor_name || ' added you to a shared plan' || COALESCE(' for ' || initcap(NEW.metadata->>'city'), '');
    ELSE
      v_title := 'Nowy wspólny plan 🗺️';
      v_body  := v_actor_name || ' dodał(a) Cię do wspólnego planu' || COALESCE(' po ' || initcap(NEW.metadata->>'city'), '');
    END IF;
  ...
  ELSIF NEW.type = 'route_liked' THEN
    IF v_en THEN
      v_title := 'Someone liked your plan ❤️';
      v_body  := v_actor_name || ' liked your plan' || COALESCE(' for ' || initcap(NEW.metadata->>'city'), '');
    ELSE
  ...
  ELSIF NEW.type = 'new_route' THEN
    IF v_en THEN
      v_title := 'New plan';
      v_body  := v_actor_name || ' published a plan' || v_city_sfx;
    ELSE
      v_title := 'Nowy plan';
      v_body  := v_actor_name || ' opublikował(a) plan' || v_city_sfx;
    END IF;
    v_url   := '/route/' || COALESCE(NEW.route_id::text, '');
  ...
  ELSIF NEW.type = 'list_starred' THEN
    -- Gwiazdka od WSPOLTWORCY przy miejscu w mojej kolekcji (notify_list_starred dedupuje
    -- do jednego na (osoba, kolekcja) na 5 minut - trzy gwiazdki pod rzad = jeden push).
    IF v_en THEN
      v_title := 'A star in your collection ⭐';
      v_body  := v_actor_name || ' starred' || COALESCE(' “' || (NEW.metadata->>'place_name') || '”', ' a place')
                 || COALESCE(' in “' || v_title_meta || '”', ' in your collection');
    ELSE
      v_title := 'Gwiazdka w Twojej kolekcji ⭐';
      v_body  := v_actor_name || ' wyróżnił(a)' || COALESCE(' „' || (NEW.metadata->>'place_name') || '”', ' miejsce')
                 || COALESCE(' w „' || v_title_meta || '”', ' w Twojej kolekcji');
    END IF;
    v_url   := '/lista/' || COALESCE(NEW.metadata->>'collection_id', '');
  ...
  ELSIF NEW.type = 'trip_reminder' THEN
    ...
    IF v_missing_photos > 0 THEN
      IF v_en THEN
        v_title := 'Your plan is waiting for photos 📸';
        v_body  := CASE
          WHEN v_missing_photos = 1 THEN 'One place on your plan' || v_city || ' still has no photo. Add it before you forget.'
          ELSE v_missing_photos || ' places on your plan' || v_city || ' still have no photo. Add them before you forget.'
        END;
      ELSE
        v_title := 'Twój plan czeka na zdjęcia 📸';
        v_body  := CASE
    ...
    ELSIF v_missing_notes > 0 THEN
      IF v_en THEN
        v_title := 'Finish your plan ✍️';
        v_body  := CASE
          WHEN v_missing_notes = 1 THEN 'Photos are complete - one place on your plan' || v_city || ' still needs a note.'
          ELSE 'Photos are complete - ' || v_missing_notes || ' places on your plan' || v_city || ' still need notes.'
        END;
      ELSE
        v_title := 'Dokończ swój plan ✍️';
        v_body  := CASE
    ...
    ELSE
      IF v_en THEN
        v_title := 'Your plan is ready to publish 🎉';
        v_body  := 'Photos and notes are complete for your plan' || v_city || '. Publish it so it shows up in Explore.';
      ELSE
        v_title := 'Plan gotowy do publikacji 🎉';
        v_body  := 'Zdjęcia i notki masz komplet' || v_city || '. Opublikuj plan, żeby pojawił się w eksploracji.';
      END IF;
    END IF;
  ...
  ELSIF NEW.type = 'trip_places_reminder' THEN
    IF v_en THEN
      v_title := 'Add places to the plan 📍';
      v_body  := v_actor_name || ' is waiting for your place suggestions'
                 || COALESCE(' for the plan for ' || initcap(NEW.metadata->>'city'), '');
    ELSE
      v_title := 'Dodaj miejsca do planu 📍';
      v_body  := v_actor_name || ' czeka na Twoje propozycje miejsc'
                 || COALESCE(' na plan po ' || initcap(NEW.metadata->>'city'), '');
    END IF;
    v_url   := '/route/' || COALESCE(NEW.route_id::text, '');
  ...
  ELSIF NEW.type = 'trip_message' THEN
    IF v_en THEN
      ...
      v_body  := v_actor_name || ' posted' || COALESCE(' in “' || v_title_meta || '”',
                   CASE WHEN COALESCE(NEW.metadata->>'city','') <> '' THEN ' in the plan for ' || initcap(NEW.metadata->>'city') ELSE '' END);
    ELSE
      v_title := 'Nowa wiadomość 💬';
      v_body  := v_actor_name || ' napisał(a)' || COALESCE(' w „' || v_title_meta || '”',
                   CASE WHEN COALESCE(NEW.metadata->>'city','') <> '' THEN ' w planie po ' || initcap(NEW.metadata->>'city') ELSE '' END);
    END IF;
    v_url   := '/route/' || COALESCE(NEW.route_id::text, '');
  ...
  ELSE -- route_used (legacy: nic go juz nie tworzy, zastapil go discovery_used)
    IF v_en THEN
      v_title := 'Someone is using your plan 🧭';
      v_body  := v_actor_name || ' saved your plan' || COALESCE(' for ' || initcap(NEW.metadata->>'city'), '');
    ELSE
