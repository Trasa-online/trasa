-- Update notify_mentioned_users function to check for existing notification before inserting
    ...
    IF NEW.mentioned_users IS NOT NULL THEN
      ...
      LOOP
        IF mentioned_user_id != route_owner_id THEN
          -- Check if notification already exists for this user+route+type combination
          SELECT id INTO existing_notification_id
          FROM public.notifications
          WHERE user_id = mentioned_user_id
            AND type = 'mention'::public.notification_type
            AND route_id = NEW.route_id
            AND actor_id = route_owner_id
          LIMIT 1;
          
          -- Only insert if no existing notification found
