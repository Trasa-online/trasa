-- Powiadomienie + push dla wlasciciela kolekcji, gdy WSPOLTWORCA wyroznil miejsce
-- (decyzja Nat 2026-09-20). Trigger na `discovery_item_stars` (SECDEF - klient nie ma
-- INSERT na `notifications`), pomija gwiazdke samego wlasciciela i dedupuje do jednego
-- powiadomienia na (osoba, kolekcja) na 5 minut. Wymaga enumu z `20260920c`.

create or replace function public.notify_list_starred()
returns trigger
language plpgsql
security definer
...
begin
  select c.user_id, c.title into v_owner, v_title
    from public.discovery_collections c where c.id = new.collection_id;
  if v_owner is null or v_owner = new.user_id then return null; end if;
  if exists (
    select 1 from public.notifications n
     where n.user_id = v_owner and n.actor_id = new.user_id and n.type = 'list_starred'
       and n.metadata->>'collection_id' = new.collection_id::text
       and n.created_at > now() - interval '5 minutes'
  ) then return null; end if;
  insert into public.notifications (user_id, actor_id, type, metadata)
  values (v_owner, new.user_id, 'list_starred',
          jsonb_build_object('collection_id', new.collection_id, 'title', v_title, 'place_name', new.place_name));
  return null;
end;
$$;
revoke execute on function public.notify_list_starred() from public, anon, authenticated;

drop trigger if exists trg_dis_notify_starred on public.discovery_item_stars;
create trigger trg_dis_notify_starred
  after insert on public.discovery_item_stars
  for each row execute function public.notify_list_starred();

-- Galaz w notify_push (pelna definicja, zeby migracja byla samowystarczalna).
