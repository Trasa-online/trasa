-- Push dla typow, ktore realnie sie dzieja: 'discovery_used' (ktos skorzystal z mojego planu)
-- i 'new_route' (obserwowany opublikowal wyjazd). Do 2026-09-01 obu NIE bylo na whitelist
-- notify_push, wiec nie wychodzil z nich zaden push - a to sa dzis najczestsze zdarzenia
-- (17 discovery_used i 12 new_route w ostatnich 14 dniach; zero pushy).
-- Na whitelist byl za to 'route_used', ktorego NIC nie tworzy (0 wierszy w calej historii bazy) -
-- typ zostal przemianowany na 'discovery_used' i nikt nie poprawil triggera. Zostawiamy go dla
-- zgodnosci wstecz, bo nie szkodzi.
--
-- UWAGA: ponizsze to ZYWA definicja z produkcji (pg_get_functiondef) + dwie nowe galezie.
-- Nie odtwarzaj tej funkcji z historii repo - bywala edytowana bezposrednio na bazie.
...
DECLARE v_actor_name TEXT; v_title TEXT; v_body TEXT; v_url TEXT; v_payload jsonb; v_secret TEXT;
  DECLARE v_missing_photos INT; v_missing_notes INT; v_city TEXT; v_city_sfx TEXT;
BEGIN
  IF NEW.type NOT IN ('group_invite','friend_request','friend_accept','route_used',
                      ...
                      'trip_places_reminder','trip_message','discovery_used','new_route') THEN
    RETURN NEW;
  END IF;
