-- Push + in-app dla przypomnienia o dokonczeniu roboczego wyjazdu (trip_reminder, 2026-08-23).
-- Dokladamy galaz 'trip_reminder' do notify_push (baza: 20260827_notify_push_trigger_secret.sql).
-- Wpis powstaje z crona (enqueue_trip_reminders) -> ten trigger wysyla push. actor_id = wlasciciel
-- (self), wiec v_actor_name sie nie uzywa w tej galezi. Wymaga enum 'trip_reminder' (migracja a).
...
BEGIN
  IF NEW.type NOT IN ('group_invite','friend_request','friend_accept','route_used',
                      'route_invite','route_liked','list_liked','list_saved','trip_reminder') THEN
    RETURN NEW;
  END IF;
