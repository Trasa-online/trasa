import { buildBusinessWelcomeHtml, buildBusinessWelcomeText } from "./welcome.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { email: rawEmail, business_name, place_id } = await req.json();
    ...
    if (!place_id || typeof place_id !== "string") throw new Error("place_id required");
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "Twoja wizytówka na Trasie jest gotowa",
        html,
        text,
      }),
    });
    ...
    console.error("[send-business-welcome]", err);
