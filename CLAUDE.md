...
- **Ramka awatara ([AvatarStarFrame](src/components/profile/AvatarStarFrame.tsx), PRÓBA 2026-09-11):** cztery brandowe gwiazdki krążące wokół awatara na własnym profilu (14 s/obrót, kontr-rotacja trzyma gwiazdki prosto, `motion-reduce` zatrzymuje). Nakładka `pointer-events-none` - włącza/wyłącza się jedną linią w `TravelerProfile`.
- **Udostępnianie miejsca (2026-09-11):** „Udostępnij to miejsce" w [SavePlaceSheet](src/components/plan-wizard/SavePlaceSheet.tsx) → arkusz `ShareCardPlace` (ta sama karta co w Miejscach, `SwipeCard scrollMode`) + link **`spontaway.com/p/<places.id>`** (serwer: [api/share.ts](api/share.ts) `t=place`, OG + karta + „Zobacz miejsce" + pasek przedpremierowy). Tylko wizytówki z bazy (UUID w `place_id`); surowy wynik Google nadal dostaje link do Google Maps. `buildShareUrl("/miejsce/<id>")` → `/p/<id>`.
