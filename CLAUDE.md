...
- **Zaproszenie:** `notify_collection_invite` (SECDEF, dedup po (osoba, kolekcja)) wstawia powiadomienie typu **`list_invite`** - nowa wartosc enuma `notification_type`, dodana OSOBNA migracja przed uzyciem. Galaz w `notify_push` (push) i wpis w `NotificationsDrawer` (ikona `Users`, przejscie na `/lista/<id>`). `add_member_to_collection` przy okazji wrzuca kolekcje do `saved_collections` zaproszonego, zeby pojawila sie u niego w „Zapisane" bez szukania.
