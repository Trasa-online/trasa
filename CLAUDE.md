...
[Auth.tsx](src/pages/Auth.tsx) (natywka, gałąź B2C): **żółte tło marki `#FDF184`, brązowy tekst `#5B2C06`**, nagłówek „Odkrywaj i zwiedzaj" w Sigmar (`font-brand`), tagline pod spodem, u góry **tasująca się talia okładek miejsc** ([WelcomeDeck](src/components/auth/WelcomeDeck.tsx): 8 losowych zdjęć userów z `place_photos` - publiczny odczyt działa przed logowaniem - jako karty 3:4 z nazwą i miastem; co 2,4 s wierzchnia karta wylatuje na bok i wraca na spód, reszta przesuwa się do przodu), karuzela USP w brązie, Apple (czarny) + Google (biały) na dole. Copy w `auth:welcome.*`. Poprzednia wersja (biel + dryfujące pomarańczowe plamy) wycofana. Ekran B2B (`?business=true`) bez zmian - niebieski.
