...
**BottomNav (native) = 5 pozycji (makieta Nat 2026-09-11):** `Eksploruj` · `Feed` · `+` (środkowy FAB) · `Miejsca` · `Profil`. **Eksploruj jest pierwsza i STARTOWA** (zimny start, po logowaniu) - nowy user nikogo nie obserwuje, więc Feed byłby pusty. Ikony: Eksploruj = **dom** (`Ikona_Home`, bo to ekran startowy), Feed = **kompas** (`Ikona_Eksploracja`) - zamienione na prośbę Nat 2026-09-11. Pasek to wąski, wyśrodkowany „pill" (HUG); target `w-[54px]` × `h-[46px]` (5 pozycji musi się zmieścić na iPhone SE: cały pill ~330 px), FAB `w-16`. Ikony z **brandowego zestawu SVG** (`public/Ikona_Home.svg`, `Ikona_Eksploracja.svg`, `Ikona_Miejsca.svg`, `Ikona_Profil.svg`, przez `NavIcon` = CSS mask + currentColor) — dom i pinezkę Nat dorysowała 2026-09-11, żeby cały pasek był w jednym języku. Plik: [src/components/layout/BottomNav.tsx](src/components/layout/BottomNav.tsx). **Zasada „jedno zadanie na widok":**
- **Feed** → `/feed` ([Feed.tsx](src/pages/Feed.tsx)). Treści **od osób, które obserwujesz** (wyjazdy + listy), jedno wydarzenie na ekranie (pełnoekranowe karty ze snapem = dawny układ eksploracji). Lista i zapytania żyją w `DiscoveryFeed` z propem `followingOnly` (filtr po `followers`). Pusty stan rozróżnia „nikogo nie obserwujesz" (CTA → szukanie ludzi) od „obserwowani nic nie opublikowali" (CTA → Eksploruj).
- **Eksploruj** → `/eksploruj` ([Explore.tsx](src/pages/Explore.tsx)) = **siatka** treści od całego świata: **wyjazdy ORAZ listy** w jednej mozaice 2-kolumnowej w stylu Pinteresta ([ExploreGrid.tsx](src/components/home/ExploreGrid.tsx)): samo zdjęcie + SAMA nazwa pod nim (bez autora), mała wyciszona ikona typu PRZED nazwą (znak „S" = wyjazd, kartka = lista; w rogu okładki „odbierała uroku"), wyjazd z galerią = karuzela z kropkami i haptyką przy zmianie zdjęcia (na starcie 2 slajdy, reszta dogrywa się po pierwszym scrollu). ⛔ NIE zmieniaj DOM karuzeli na touchstart/pointerdown - WebKit przerywa wtedy przewijanie i dostarcza click (swipe otwierał wyjazd). ⛔ NIE kolumny CSS (`columns-2`): WebKit nie malował prawej kolumny przy karuzeli - są dwie kolumny flex z rozdziałem naprzemiennym. Przypięta wyszukiwarka zostaje tu (wyniki nadal renderuje `DiscoveryFeed` w trybie `searchOnly`). **Przełącznik Trasy|Miejsca w belce USUNIĘTY** (`hideModeToggle`). Flaga `SHOW_ZESTAWIENIA` = `true` od 2026-09-11 (listy wróciły do eksploracji i wyszukiwarki).
...
6. **Mapa „Wybierz z mapy" ([PlaceMapPicker](src/components/route/PlaceMapPicker.tsx)) pyta Google WYŁĄCZNIE za jawną intencją (decyzja Nat 2026-09-11):** tapnięcie w etykietę lokalu na podkładzie (`placeId` z Maps JS za darmo → proxy `action: "placeid"`, pola podstawowe, cache 7 dni w `place_details_cache`) albo „+" z wpisaną nazwą (jedno `textsearch` z `latitude/longitude` = bias 3 km, kandydaci od najbliższych pinezce). ⛔ NIE dodawaj rozpoznawania po postoju pinezki / przy zoomie (Nearby przy każdym przybliżeniu) - odrzucone tego samego dnia: koszt przy przeglądaniu mapy i lokale niezwiązane z celem. Miejsce ręczne (bez dopasowania) ma `address: ""`, nigdy `null` (`pins.address` NOT NULL).
...
**Arkusze dolne są PŁYWAJĄCE (2026-09-11, wzór FYI):** 8 px od lewej, prawej i dołu, zaokrąglone z każdej strony - daje to sam wariant `bottom` w [sheet.tsx](src/components/ui/sheet.tsx) (`inset-x-2 bottom-2 rounded-3xl`); ręczne panele (`fixed inset-0 flex items-end`) mają `w-[calc(100%-16px)] mx-2 mb-2 rounded-3xl`. Nowy panel rób tak samo, NIE `inset-x-0 bottom-0 rounded-t-3xl`.
