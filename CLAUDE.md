...
- **Ikona aplikacji (home screen) = NOWE LOGO od Nat (2026-09-15):** jeden plik `public/Logo_Spontaway.jpg` (1024) jest ŹRÓDŁEM PRAWDY - **płaskie żółte tło `#FDF184`** + **pomarańczowy znak `#F75708`**: faliste „S" z **GWIAZDKĄ** w prawym górnym rogu (wcześniej stała tam pinezka). Zastąpiło to żółto-złoty gradient z 2026-08-12 - ikona jest teraz PŁASKA, bez gradientu. **Wszystko generuje `python3 scripts/gen_app_icon.py`**: master `public/App icon IOS.png`, PWA `icon-192/512`, `apple-touch-icon` 180, `favicon.png` 48 + `favicon.ico`, **`Avatar_Trasa.png`** (domyślny awatar = ta sama ikona, `DEFAULT_AVATAR`) oraz `ios/.../AppIcon-512@2x.png`. Zmienia się logo → podmień TEN JEDEN plik i odpal skrypt, nie edytuj pochodnych ręcznie. ⛔ To NIE dotyczy **presetów awatarów** (`public/avatars/preset-*`) - tam są same kolory, bez znaku (decyzja Nat). **Ikona jest DWUKOLOROWA co do piksela i Xcode jej NIE skaluje (2026-09-15, drugie zgłoszenie Nat: „S ma na sobie jakiś gradient").** Dwa źródła fałszywego cieniowania, oba usunięte: (1) źródłem jest JPEG, więc chroma 4:2:0 zostawiała wzdłuż całego „S" obwódkę odcieni pośrednich - `clean_two_tone()` przemalowuje każdy piksel na mieszankę DOKŁADNIE `#FDF184` i `#F75708`, więc w plikach nie ma już żadnego innego koloru (206 odcieni → 39, zero poza linią) + doklejony profil sRGB; (2) katalog miał JEDEN master 1024, a `actool` dorabiał resztę **ringującym** resamplerem - w ikonie 120 px, którą dostawał telefon, było **899 pikseli poza kolorami marki**: ciemniejsze `(247,81,3)` tuż wewnątrz krawędzi i jaśniejsze `(253,248,138)` tuż obok, czyli ciemny rant wzdłuż całego znaku. Teraz `write_ios_iconset()` kładzie w katalogu **komplet 13 rozmiarów** (iPhone + iPad + marketing), więc `actool` tylko kopiuje: sprawdzone `actool --compile`, 0 pikseli poza kolorami marki. ⛔ Nie wracaj do pojedynczego mastera w `AppIcon.appiconset` i nie skaluj ikony zwykłym `LANCZOS` - używaj `resize_two_tone()`, które rzutuje piksele z powrotem na odcinek żółty-pomarańcz. ⚠️ Po zmianie ikony iOS trzyma starą w cache springboarda - żeby zobaczyć nową, **skasuj apkę z telefonu i zbuduj od nowa**, sam Cmd+R nie wystarczy.
