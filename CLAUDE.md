...
**Widok wyjazdu i listy - nagłówek (redesign Nat 2026-09-13, makieta „Łódzki citybreak"):** wspólny moduł [TripHeaderChips.tsx](src/components/route/TripHeaderChips.tsx). Belka: **wstecz = chevron w BIAŁYM kółku z szarym obrysem** (`bg-white border-border`; peachy odrzucone), **autor jako pigułka** (`AuthorPill`: peachy, CZARNY `@nick`, awatar z ramką - nakładka ZOSTAJE) + po prawej: gość = **flaga zgłoszenia** (`ReportContentSheet` z własnym triggerem; wcześniej link „Zgłoś" pod listą miejsc); właściciel/uczestnik = **menu „…" w BELCE** (białe kółko z delikatnym cieniem `shadow-[0_1px_5px_rgba(0,0,0,0.12)]`; w wyjeździe obok żółtego kółka udostępniania) - przy tytule nie ma już żadnych guzików poza polubieniem. **Polubienie = BRANDOWE SERCE na wysokości tytułu** (`BrandHeart` w [BrandHeart.tsx](src/components/BrandHeart.tsx) = inline svg z `public/Ikona_serce.svg`, kontur = nie polubione / pełne = polubione, jak `BrandStar`; NIE lucide `Heart`; tylko wyjazd, tylko gość; licznik gdy > 0). Gwiazdka zostaje wyłącznie dla „topki" (`BrandStar` w [BrandStar.tsx](src/components/BrandStar.tsx) = brandowa gwiazdka z konturem, gdyby była potrzebna). **Daty wyjazdu pod chipami = sama informacja** (ikona kalendarza + zakres, bez ołówka); ustawianie/zmiana dat żyje w menu „…" w belce („Dodaj daty wyjazdu" / „Zmień daty wyjazdu", tylko właściciel; wiersz „Dodaj daty" pod tytułem usunięty 2026-09-13). **Udostępnianie = brandowe ŻÓŁTE kółko z brązową ikoną bezpośrednio NA PRAWO od dolnego CTA** („Zapisz ten wyjazd" u gościa; na liście obok „Zapisz tę listę" / „Dodaj nowe miejsce"). **Chipy dni** w wyjeździe: zaznaczony = peachy `#FCEDE3` + brązowy tekst (NIE pomarańcz, NIE żółty). Pod tytułem **chipy z SAMYM OBRYSEM** (`HighlightChips`, bez wypełnienia, obrys **1,5 px** - `border-2` Nat uznała za zbyt gruby, brązowy tekst): miasto (obrys brąz `#5B2C06`; `route.city` przed krajami), liczba miejsc (obrys łosoś `#F0A583`; copy z polską odmianą i wielką literą „15 Miejsc", a na liście **„7 / 15 Miejsc"** = odwiedzone / wszystkie: moje odwiedziny na własnej liście, autora na cudzej), wyróżnione gwiazdką (obrys żółty `#FDF184`, tylko gdy > 0) - ikony chipów to BRANDOWE SVG (`Ikona_Miejsca` w kolorze `#CC3434`, `Ikona_Gwiazdka` pomarańczowa) większe niż chip i **wychodzące poza pigułkę** (absolutne, 36 px). Opis wyjazdu/listy `text-[15px] text-foreground/80`. Wiersz miejsca ([RoutePlaceRow](src/components/route/RoutePlaceRow.tsx)): miniatura 2:3 `w-16 h-24 self-start` (wariant rozciągany odrzucony), chip kategorii peach + brąz, akcje w kółkach 40 px: Google · **dodaj zdjęcie** (pomarańczowe kółko, brandowy aparat `CAMERA_ICON` = `public/aparat.svg` w żółtym `#FDF184` + mały plus) · „…" z resztą (w tym „dodaj notkę" - osobne kółko notki odrzucone jako zagęszczające) - zdjęcie wychodzi z `menuExtras` po kluczu `photo`, callery bez zmian. **„Odwiedzone" = ŻÓŁTA pigułka (`#FDF184`) POD miniaturką: awatar + podwójny ptaszek (`CheckCheck`), bez napisu** - na cudzej liście awatar autora, na własnej mój; przełącznik „byłem tu" na WŁASNEJ liście siedzi w menu „…" (2026-09-13; wcześniej osobne kółko z ptaszkiem i pigułka z napisem). Kolumna treści wiersza ma `min-h-24` i składa akcje na dole (`mt-auto`) - miejsce bez notki i zdjęć (stan zero) jest zwartym wierszem z kółkami na wysokości dolnej krawędzi miniatury. **Zakładka „zapisz" = brandowa `SAVE_ICON` (`Ikona_Zapisane.svg`)** w wierszu, menu i CTA „Zapisz ten wyjazd / tę listę" - NIE lucide `Bookmark`.
...
4a. **Wizytówka z dodawania miejsc (2026-09-13):** `PlaceSwiperDetail` otwarta z [route/AddPlaceSheet](src/components/route/AddPlaceSheet.tsx) i [CreateFlowSheet](src/components/create/CreateFlowSheet.tsx) dostaje `onAdd`/`added` = toggle z wiersza, z którego ją otwarto → drugi guzik **„Dodaj to miejsce"** (primary, plus) obok „Zapisz to miejsce" (wtedy szary secondary); po dodaniu wizytówka się zamyka, „Dodano" = już zaznaczone / już w wyjeździe (guzik nieaktywny, odznaczenie na wierszu).
