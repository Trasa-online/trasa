...
- **Eksploruj** → `/eksploruj` ([Explore.tsx](src/pages/Explore.tsx)) = **siatka** treści od całego świata: **wyjazdy ORAZ listy** w jednej mozaice 2-kolumnowej w stylu Pinteresta ([ExploreGrid.tsx](src/components/home/ExploreGrid.tsx)): samo zdjęcie + SAMA nazwa pod nim (bez autora), ikona typu w rogu (znak „S" = wyjazd, kartka = lista), wyjazd z galerią = karuzela z kropkami (kolejne zdjęcia dociągane dopiero po dotknięciu). ⛔ NIE kolumny CSS (`columns-2`): WebKit nie malował prawej kolumny przy karuzeli - są dwie kolumny flex z rozdziałem naprzemiennym. Przypięta wyszukiwarka zostaje tu (wyniki nadal renderuje `DiscoveryFeed` w trybie `searchOnly`). **Przełącznik Trasy|Miejsca w belce USUNIĘTY** (`hideModeToggle`). Flaga `SHOW_ZESTAWIENIA` = `true` od 2026-09-11 (listy wróciły do eksploracji i wyszukiwarki).
