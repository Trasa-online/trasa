...
[Auth.tsx](src/pages/Auth.tsx) (natywka, gałąź B2C): **żółte tło marki `#FDF184`, brązowy tekst `#5B2C06`**, nagłówek „Odkrywaj i zwiedzaj" w Sigmar (`font-brand`) BEZ claimu pod spodem (zdjęty 2026-09-14; tagline zostaje w marketingu), u góry **tasująca się talia okładek** ([WelcomeDeck](src/components/auth/WelcomeDeck.tsx): karty 3:4 176 × 234 px bez obrysu - okładki MIEJSC ze zdjęć userów z `place_photos` przeplatane kafelkami KOLEKCJI (kolorowe tło z palety `listThemes` + mini-siatka 3 miejsc + tytuł, publiczne kolekcje `visited`); oba źródła mają publiczny odczyt, więc działają przed logowaniem; co 2,4 s wierzchnia karta wysuwa się w bok i CHOWA POD spód talii - bez zanikania, z-index spada w połowie lotu, gdy karta jest już poza talią), karuzela USP w brązie, Apple (czarny) + Google (biały) na dole. Copy w `auth:welcome.*`. Poprzednia wersja (biel + dryfujące pomarańczowe plamy) wycofana. Ekran B2B (`?business=true`) bez zmian - niebieski.
...
- **Czego NIE zrobiono:** nic z tej rodziny - notki i zdjecia sa juz per-uczestnik. Patrz memory `project_list_cocreation_architecture`.
